<template>
  <div id="detail">
    <template v-if="task">
      <h2 :style="task.plan.state[stateKey] && task.plan.state[stateKey].finished?'text-decoration-line: line-through;':''">{{ task.title
        }}</h2>
      <h6>{{ '创建时间：'+ require('moment')(task.createTime,'YYYYMMDDHHmmss').format('YYYY-MM-DD HH:mm:ss') }}</h6>
      <h6 v-if="task.plan.state[stateKey] && task.plan.state[stateKey].finished">
        {{'完成时间：'
      + require('moment')(task.plan.state[stateKey] && task.plan.state[stateKey].finishTime,'YYYYMMDDHHmmss')
          .format('YYYY-MM-DD HH:mm:ss')}}
      </h6>
      <b-button variant="danger"
                size="sm"
                @click="cancelPlan"
                v-if="task.plan.state[stateKey] && !task.plan.state[stateKey].finished">
        取消计划
      </b-button>
      ...
      <b-form style="margin-top: 10px">
        <b-form-textarea
            ...
            :readonly="task.plan.state[stateKey] && task.plan.state[stateKey].finished">
        </b-form-textarea>
